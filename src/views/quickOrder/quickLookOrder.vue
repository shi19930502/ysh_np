				...
				<div class="list_style">订单号：<span>{{d.orderUmber.slice(0,12)}}</span><span  style="color: #FF3D15;font-weight: bold;font-size: 15px;">{{d.orderUmber.slice(12,16)}}</span></div>
					<div class="flex flex-x-between flex-y-center">
						<div class="flex">
							<template v-for="orderitem,key in d.orderitemList">
								<div class="product-img" v-if="key<4">
									<img v-if='orderitem.skuPicUrl&&orderitem.skuPicUrl!=null'  :src="$root.imgURL+orderitem.skuPicUrl" alt="" />
									<img v-else-if='orderitem.productPicUrl&&orderitem.productPicUrl!=null' :src="$root.imgURL+orderitem.productPicUrl" alt="" />
						...
						<div class="tar">
							<p class="price">
								<!--sortingOrderTotal-->
								<span v-if="d.orderStatusId==0||d.orderStatusId==1||d.orderStatusId==6||d.orderStatusId==7 ">{{'￥'+d.orderTotalForDecimal}}</span>

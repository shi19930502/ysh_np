						...
						<div class="flex flex-x-between flex-y-center">
							<div class="flex">
								<div class="img">
									...
									<img src="../../assets/imgs/img_moren@2x.png">
								</div>
								<div>
									<!--<h3>{{d.productName}}</h3>-->
	...
	export default {
		...
		data() {
			return {
				...
				payParams: {
					payType: 0,
					distributionType: 1,
					distributionDate: this.GetDateStr(0).date,
		...
		created() {
			...
			if(localStorage.getItem('settlementParam')) {
				this.settlementGoods = JSON.parse(localStorage.getItem('settlementParam')).settlementGoods
			} else {
				this.$router.push({
					name: 'Home'
				})
			...
			for(var i in this.settlementGoods) {
				if(this.settlementGoods[i].promotionPrice > 0 && this.settlementGoods[i].promotionBasePrice > 0) {
					this.orderTotal += this.settlementGoods[i].quantity * (this.settlementGoods[i].promotionPrice * 100)
				} else {
					this.orderTotal += this.settlementGoods[i].quantity * (this.settlementGoods[i].price * 100)
				}
				this.aQuantity += this.settlementGoods[i].quantity
			}
		},
		mounted() {},
		methods: {
			...
			setPayParams(storeInfo) {
				...
				if(this.payParams.distributionType == 0) { // 如果店铺不支持当天配送distributionType == 0，只能选择第二天的日期
					...
					this.payParams.distributionTimeBegin = storeInfo.distributionStart < 10 ? `0${storeInfo.distributionStart}:00` : `${storeInfo.distributionStart}:00`
					this.payParams.distributionTimeEnd = storeInfo.distributionStart + 1 < 10 ? `0${storeInfo.distributionStart+1}:00` : `${storeInfo.distributionStart+1}:00`
				} else {
			...
			toOrderCheckoutSuccess() {
				...
				if(this.createOrderObj.shippingAdd == undefined || this.createOrderObj.shippingAddId == undefined) {
					this.$dialog.alert({
						mes: '请选择收货地址~~'
					})
					return false
				...
				for(var i in this.settlementGoods) {
					...
					if(this.settlementGoods[i].promotionPrice > 0 && this.settlementGoods[i].promotionBasePrice > 0) {
						o.oriPrice = this.settlementGoods[i].promotionBasePrice * 100
						o.price = this.settlementGoods[i].promotionPrice * 100
					} else {
						o.oriPrice = this.settlementGoods[i].basePrice * 100
						o.price = this.settlementGoods[i].price * 100
					}
				...
				this.postRequest('quotation/listWithCount', {
					...
					if(resp.data && resp.data.state == 0) {
						if(resp.data.aaData && resp.data.aaData[0]) {
							if(this.createOrderObj.orderTotal / 100 < resp.data.aaData[0].startAmount) {
